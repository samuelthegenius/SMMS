-- Direct SQL to fix function ambiguity
-- Run this directly in the Supabase SQL Editor

-- First, find all function signatures
SELECT proname, proargtypes::text FROM pg_proc WHERE proname = 'register_secure_user';

-- Drop functions by their specific OID (object ID)
DO $$
DECLARE
    func_record RECORD;
BEGIN
    FOR func_record IN 
        SELECT oid FROM pg_proc WHERE proname = 'register_secure_user'
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.oid::regprocedure || ' CASCADE';
    END LOOP;
END $$;

-- Now create definitive version
