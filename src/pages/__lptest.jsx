...
export default function LandingPage() {
    ...
    // Logged-in users should never see any landing-page content, not even a
    // loader: redirect during render via <Navigate>, so the router swaps to
    // /dashboard before this component paints anything (no flash).
    if (user) {
        return <Navigate to="/dashboard" replace />;
    }

    // Still resolving auth state but destination unknown yet: show the
    // landing page's own skeleton (matches the page that will actually render
    // if it turns out there's no logged-in user).
    if (initializing) {
