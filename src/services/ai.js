...
export async function autoCategorizeWithFallback(title, description = '', facilityType = 'Other', confidenceThreshold = 0.7, priorityThreshold = 0.6) {
  try {
    ...
    if (result && result.error) {
      throw new Error(result.message || result.error || 'AI categorization unavailable');
    }
    ...
    // Strict mode: Instead of faking a fallback, bubble the error up so the UI 
    // can show a proper error toast ("AI categorization failed. Please select manually.")
    throw error;
  }
}
